...
#### Advanced: Cross-Agent Context Handoff

Seamlessly inherit conversation history, decisions, and artifacts from one agent (e.g. Codex / GPT) into another agent (e.g. Claude Code):

```bash
# 1. Have Codex perform initial architecture analysis and API design
agentmesh run codex "Design high-concurrency WebSocket gateway protocol and schemas"
# -> Generates TASK_ID: a1b2c3d4-...

# 2. Launch Claude Code, automatically inheriting Codex's context, decisions, and artifacts
agentmesh run claude "Implement client connection pool and reconnect logic based on the spec" --from-task a1b2c3d4-...
```
...
| `agentmesh run <agent> <prompt> [--from-task <id>] [--from-context <id>]` | Run a task with optional cross-agent context inheritance |
